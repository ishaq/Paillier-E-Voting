...
def kick_off():
    ...
    while True:
        ...
        voter_pin = input("Enter your PIN: ")

        vote = -1
        while vote < 0 or vote > (config.NUM_CANDIDATES - 1):
            vote_str = input("Which candidate are you voting for (0-{}): ".format(config.NUM_CANDIDATES - 1))
            try:
                vote = int(vote_str)
            except ValueError:
                continue
        ...
        choice = input("Enter 'y' to submit, any other key to start over: ")
